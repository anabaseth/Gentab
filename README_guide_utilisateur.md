...
- [Les poids](#les-poids)
- [Colonnes multiples et min/max](#colonnes-multiples-et-minmax)
...
1. **Ouvrez Gentab** — double-cliquez sur `gentab.html`.
2. **Chargez votre fichier** — cliquez sur **Charger** et sélectionnez votre fichier `.gntb`.
3. **Choisissez une séquence** — sélectionnez-la dans la liste déroulante de l'onglet **⚄ Tirer**.
...
> Gentab garantit que **chaque combinaison est unique** dans un tirage : vous n'obtiendrez jamais deux fois « Corbin Ashcroft » dans le même résultat. Une valeur isolée (ex. « Corbin ») peut en revanche apparaître plusieurs fois si elle est couplée à des noms de famille différents.
...
| Réglage | Comportement | Exemple de résultat |
|---------|-------------|---------------------|
...
> Quand plusieurs valeurs sont générées pour une même cellule, elles s'affichent chacune sur **sa propre ligne** dans le tableau de résultats.

---

## Créer une table

1. **Cliquez sur ＋ à côté de « Tables »** — une nouvelle table vide est créée et sélectionnée.
2. **Donnez un nom à votre table** — ce nom sera l'en-tête de colonne dans les résultats. Ex. : *Prénom*, *Lieu*, *Type d'objet*.
3. **Ajoutez vos entrées** — cliquez sur **+ Ajouter une ligne**, renseignez le contenu et le poids. Répétez pour chaque entrée. Le pourcentage se calcule automatiquement.

> 💡 **Import en masse :** vous avez déjà une longue liste ? Cliquez sur **Importer des lignes** et collez votre liste — une entrée par ligne. Vous pouvez aussi préciser le poids en ajoutant `;10` après chaque entrée (ex. `Épée;10`).

> 🖱️ **Réordonner :** glissez-déposez les lignes (par la poignée ⠿ à gauche) pour les réorganiser. L'ordre n'affecte pas les probabilités.
...
2. **Donnez un nom et éventuellement une description** — le nom s'affiche dans la liste déroulante de l'onglet Tirer.
...
4. **Reliez chaque item à une table** — pour chaque item, cliquez sur **+ Référence**, choisissez *Table*, sélectionnez votre table et réglez le min/max.

> Le **titre de l'item** sera l'en-tête de colonne dans le tableau de résultats. Donnez-lui un nom parlant : *Prénom*, *Lieu*, *Butin*…
...
Vous créez une séquence « Butin » avec un item « Objet » qui référence les deux sous-séquences avec le même poids. Selon celle choisie, le tableau affichera les colonnes correspondantes.

> Les colonnes issues d'une séquence imbriquée s'affichent en *violet italique* dans le tableau de résultats.
...
> Un fichier `.gntb` est simplement du **texte**. Vous pouvez l'ouvrir avec n'importe quel éditeur de texte (Notepad, VS Code…) pour l'inspecter ou le modifier manuellement si vous le souhaitez.

---

## Faire un tirage

1. **Assurez-vous qu'un fichier est chargé** — le badge en haut affiche ✦ vert si tout est en ordre.
2. **Sélectionnez une séquence** — choisissez-la dans la liste déroulante. Sa description s'affiche en dessous si elle en a une.
...
- Les colonnes issues de séquences imbriquées en *violet italique*.
...
> 💡 Chaque clic sur **⚄ Tirer** *ajoute* de nouveaux résultats au tableau existant. Cliquez sur **Effacer** d'abord si vous voulez repartir de zéro.
...
Vérifiez que le fichier est bien un fichier texte valide (il peut s'ouvrir avec un éditeur de texte). Si vous avez modifié le fichier manuellement, une virgule manquante ou un guillemet non fermé suffit à le rendre illisible. Des outils en ligne comme *jsonlint.com* permettent de valider le contenu gratuitement.
...
Oui. Un fichier `.gntb` est du texte standard — vous pouvez l'ouvrir et le modifier avec Notepad, VS Code, ou n'importe quel éditeur. L'éditeur intégré de Gentab reste cependant recommandé car il valide votre fichier en temps réel et évite les erreurs.

---

**J'ai un fichier d'une ancienne version de Gentab. Est-il compatible ?**
...
*THE BEER-WARE LICENSE — Do whatever you want with this. Buy me a beer if we meet.*